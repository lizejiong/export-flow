...
test('merges progress event without moving progress backwards', () => {
  const event: TaskProgressEvent = { eventType: 'task.progress', taskId: 1, status: 'PROCESSING', stage: 'QUERYING_WRITING', currentRunNo: 0, progress: 10, expectedCount: 100, exportedCount: 10, updatedAt: '' }
...
test('ignores processing event after terminal state', () => {
  const event: TaskProgressEvent = { eventType: 'task.progress', taskId: 1, status: 'PROCESSING', stage: 'QUERYING_WRITING', currentRunNo: 0, progress: 50, expectedCount: 100, exportedCount: 50, updatedAt: '' }
...
test('resets progress when a newer manual run starts', () => {
  const event: TaskProgressEvent = {
    eventType: 'task.progress', taskId: 1, status: 'PROCESSING', stage: 'PREPARING', currentRunNo: 1,
