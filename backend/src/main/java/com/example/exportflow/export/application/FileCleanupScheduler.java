...
public class FileCleanupScheduler {
    ...
    public FileCleanupScheduler(ExportTaskMapper taskMapper, ExportRunMapper runMapper, LocalFileStorage storage) {
    ...
    public void cleanup() {
        ...
        for (ExportTask task : taskMapper.findExpired(now, 100)) {
            try {
                ...
                if (task.getCurrentRunId() != null) runMapper.markExpired(task.getCurrentRunId(), now);
